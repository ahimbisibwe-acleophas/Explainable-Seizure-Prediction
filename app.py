...
def explain_lime():
    try:
        ...
        sample = X.iloc[0].values  # 1D array

        # Model wrapper for LIME - returns probs for two classes
        def predict_fn(x):
            x_reshaped = x.reshape(x.shape[0], 1, x.shape[1])
            preds = model.predict(x_reshaped)
            return np.hstack([1 - preds, preds])  # Prob for class 0 and class 1
...
def explain_shap():
    try:
        ...
        # Sanity check for feature length
        assert len(shap_values[0][0]) == sample.shape[1], "Feature length mismatch"
